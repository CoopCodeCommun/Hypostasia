...
**Statut** : **SPÉCIFIÉE, NON CODÉE.** Aucun mode d'édition, aucun raccourci
d'édition, aucun endpoint de lot. **La voie technique n'est pas choisie** : voir § 11.
...
> endroit du code de production à employer l'autre vocabulaire est
> `moteur_structure.py:475-488` — **et il est en défaut** : voir
> `PLAN/TODO/2026-08-23-la-fusion-de-deux-tours-garde-le-mauvais-locuteur.md`. **Ce
> défaut se corrige AVANT tout ce que cette spec décrit.**
...
2. **`marginalia.js:290-297` est un danger direct.** Un **second** écouteur `Échap`,
   ...
   `.editeur` et `.est-en-edition` sont la convention de `_editeur_en_place.html` et
   de `maquette.css:2040-2044`. Si le mode réutilise cette convention — le chemin
   naturel —, **une frappe d'`Échap` efface des heures de sténotypie**. Cet écouteur
   doit être fusionné dans la cascade ou neutralisé **avant** le premier commit du
   mode.
...
> **Préalable non négociable** : corriger d'abord
> `PLAN/TODO/2026-08-23-la-fusion-de-deux-tours-garde-le-mauvais-locuteur.md`.
> `_fusionner_les_provenances` lit trois clés que l'ingestion n'écrit pas — la fusion
> de deux tours **conserve donc silencieusement le locuteur ET la borne de fin du
> premier**. Son test est vert sur un contrat mort. Spécifier des gestes au-dessus
> d'un contrat de provenance faux n'a aucun sens.
...
2. texte vide ⇒ **masquer** (§ 5.3) ;
3. sinon ⇒ `reconcilier_les_portions_de_l_element`, **verrou de ligne**.
...
**La voie technique.** Trois sont en concurrence, départagées sur mesure par
`PLAN/TODO/2026-08-23-le-prototype-blocknote-sur-notre-back.md` : **A** le clavier
maison, **B** BlockNote, **C** le champ unique. Cette spec décrit ce que l'utilisateur
peut faire ; elle ne dit pas comment. **Les trois doivent servir le même contrat** —
c'est ainsi qu'on les compare.
...
2. **l'endpoint de lot** (§ 7) ;
3. **un endpoint qui rend UN bloc**, pour remplacer `lectureReload` par un swap ciblé.
   Ce troisième point avait été perdu lors de l'absorption des notes, et il est
   **structurant** : `hypostasia.js:482-513` fait `zoneLecture.innerHTML = …`, et
   `lectureReload` part à **toute** opération d'élément, **y compris celle d'un autre
   utilisateur** (`views_element.py:116, 132`). Sans swap ciblé, **le premier collègue
   qui masque un bloc détruit la session d'édition en cours**, sans avertissement ni
   sauvegarde.
...
- les labels **non textuels** : sur 1 129 éléments, **20 `table`** (dont **2** portent
  ...
  éditables, ou en lecture seule dans le mode ?**

> **Le cas transcription n'est pas mesurable sur cette base** :
> `provenance__has_key='locuteur'` rend **0 élément** — aucune transcription n'y est
> ingérée. Tout ce que cette spec dit de l'audio vient du **code**. **Ingérer une vraie
> transcription est le premier geste du chantier**, juste après le correctif de fusion.
