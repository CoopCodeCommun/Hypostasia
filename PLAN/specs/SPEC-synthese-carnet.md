...
**Étalon** : `front/static/front/maquettes/corpus.html` — toute divergence entre cette spec et la
maquette est un défaut de l'une des deux, à trancher avant de coder
**Statut** : **IMPLÉMENTÉE** — phases A à I codées, migrées et à l'écran, § 6
(mise à jour par `section_ops`) compris. Le texte reste celui de la proposition ;
il se lit comme la justification du code livré, pas comme un travail à faire.
...
Étalon : `front/static/front/maquettes/corpus.html`, vérifié par `scripts/verifier_les_maquettes.py`
